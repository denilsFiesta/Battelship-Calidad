...
public class BattleshipController {
    ...
    private int executeCurrentCommand(String[] atoms) {
        try {
            ...
            FiringMode mode;
            if (atoms.length == 3) {
                parameters = App.parse(Arrays.copyOfRange(atoms, 1, 3));
                mode = FiringMode.TORPEDO_FIRING_MODE;
            } else {
                // length == 2
                parameters = App.parse(Arrays.copyOfRange(atoms, 0, 2));
    ...
    private boolean correctInput(String[] atoms) {
        ...
        if (atoms.length == 3) {
            if (!Objects.equals(atoms[0], "T") && !Objects.equals(atoms[0], "t"))
                return false;
            try {
                App.parse(Arrays.copyOfRange(atoms, 1, 2));
            } catch (NumberFormatException ex) {
                return false;
            }
        } else {
            try {
                App.parse(Arrays.copyOfRange(atoms, 0, 1));
            } catch (NumberFormatException ex) {
                return false;
            }
        }
